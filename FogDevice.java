...
public class FogDevice extends PowerDatacenter {
			...
			if (st.getSourceAp() != null && (!st.isLockedToMigration())) {
				if (method == "MIGRROR") {
					if(!NextVM && a.indexOf(st.getVmLocalServerCloudlet()) < a.size()-1) {
						...
						send(getNextSC(st).getId(),2000, MobileEvents.MIGRROR, st);
				}}
				else {
	...
	protected void processTupleArrival(SimEvent ev) {
		...
		if (appToModulesMap.containsKey(tuple.getAppId())) {
			if (appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName())) {
				...
				List <MobileDevice>  st = MobileController.getSmartThings();
	...
	public void Migrror(SimEvent ev) {
		...
		sendNow(mobileController.getId(), MobileEvents.APP_SUBMIT_MIGRROR, app);
	}

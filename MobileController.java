...
public class MobileController extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		...
		case MobileEvents.APP_SUBMIT_MIGRROR:
			processAppSubmitMigrror(ev);
			break;
				...
				if (!st.isLockedToHandoff()) {
					...
					if (distance >= MaxAndMin.AP_COVERAGE - MaxAndMin.MAX_DISTANCE_TO_HANDOFF
						...
						if (index >= 0) {// index isn't negative
							...
							int delayConnection = 1; // connection between SmartT and ServerCloudlet
								...
								/*if (st.isPostCopyStatus() && !st.isMigStatus()) {
									if (!st.isMigStatusLive()) {
										...
										send(st.getVmLocalServerCloudlet().getId(), 
												0//newMigTime + delayProcess
											, MobileEvents.SET_MIG_STATUS_TRUE, st);
									}
								}*/
								
							}
							int srcId = st.getVmLocalServerCloudlet().getId();
							int entityId = st.getDestinationServerCloudlet().getId();
							Double delay = 1.0;
							...
							//handoffLocked+delay ,
							sendNow(st.getSourceAp().getId(), MobileEvents.START_HANDOFF,st);
							//handoffTime
							sendNow(st.getDestinationAp().getId(),// handoffLocked,
	...
	private void processAppSubmitMigrror(SimEvent ev) {
		Application application = (Application) ev.getData();
